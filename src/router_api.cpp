...
SedsResult seds_router_set_local_network_time(SedsRouter *r, bool has_year, int32_t year, bool has_month, uint8_t month,
                                              bool has_day, uint8_t day, bool has_hour, uint8_t hour, bool has_minute,
                                              uint8_t minute, bool has_second, uint8_t second, bool has_nanosecond,
                                              uint32_t nanosecond) {
  if (r == nullptr)
...
SedsResult seds_router_set_local_network_datetime(SedsRouter *r, int32_t year, uint8_t month, uint8_t day, uint8_t hour,
...
SedsResult seds_router_log_queue_typed(SedsRouter *r, SedsDataType ty, const void *data, size_t count, size_t elem_size,
                                       SedsElemKind kind) {
...
SedsResult seds_router_receive_serialized(SedsRouter *r, const uint8_t *bytes, size_t len) {
  ...
  std::scoped_lock lock(r->mu);
  if (frame->envelope.ty != SEDS_DT_DISCOVERY_ANNOUNCE && frame->envelope.ty != SEDS_DT_DISCOVERY_TIMESYNC_SOURCES &&
...
SedsResult seds_router_transmit_message_queue(SedsRouter *r, const SedsPacketView *view) {
  ...
  std::scoped_lock lock(r->mu);
  if (all_endpoints_local(*r, pkt)) {
    return seds::enqueue_rx(r->rx_queue, r->rx_queue_bytes, {std::move(pkt), std::nullopt, {}}) ? SEDS_OK
...
SedsResult seds_router_process_rx_queue_with_timeout(SedsRouter *r, uint32_t) {
  ...
  while (auto item = seds::pop_rx(r->rx_queue, r->rx_queue_bytes)) {
    if (!item->wire_bytes.empty()) {
      ...
      if (frame && frame->envelope.ty != SEDS_DT_DISCOVERY_ANNOUNCE &&
          frame->envelope.ty != SEDS_DT_DISCOVERY_TIMESYNC_SOURCES &&
