...
std::unordered_map<uint64_t, int32_t> expected_end_to_end_destinations(const SedsRouter &r, const PacketData &pkt) {
  ...
  for (const auto &[side_id, route] : r.discovery_routes) {
    ...
    for (const auto &[sender, sender_state] : route.announcers) {
      if (!is_end_to_end_destination_sender(sender)) {
        continue;
      }
...
template <typename OwnerT> void handle_ack_impl(OwnerT &owner, int32_t side_id, uint32_t ty, uint32_t ack) {
...
template <typename OwnerT> void handle_packet_request_impl(OwnerT &owner, const int32_t side_id, const uint32_t ty,
                                                           const uint32_t seq) {
...
template <typename OwnerT> void queue_reliable_ack(OwnerT &owner, const int32_t side_id, const uint32_t ty,
                                                   const uint32_t seq) {
  if (side_id < 0 || static_cast<size_t>(side_id) >= owner.sides.size()) {
    return;
  }
  ...
  enqueue_tx_front(owner.tx_queue, owner.tx_queue_bytes,
                   {make_reliable_control_packet(SEDS_DT_RELIABLE_ACK, ty, seq, owner.now_ms(), control_sender(owner)),
                    std::nullopt,
                    side_id, false});
}

template <typename OwnerT> void queue_reliable_packet_request(OwnerT &owner, const int32_t side_id, const uint32_t ty,
                                                              const uint32_t seq) {
  if (side_id < 0 || static_cast<size_t>(side_id) >= owner.sides.size()) {
    return;
  }
...
template <typename OwnerT> void do_reliable_timeouts(OwnerT &owner) {
    ...
    for (auto pending_it = sent.pending_destinations.begin(); pending_it != sent.pending_destinations.end();) {
      ...
      if (route_it != r.discovery_routes.end() && r.now_ms() - route_it->second.last_seen_ms <= kDiscoveryTtlMs) {
        for (const auto &[sender, sender_state] : route_it->second.announcers) {
          ...
          if (sender_hash(sender) == pending_it->first) {
            found = true;
            break;
          }
        }
...
SedsResult dispatch_local_packet_handlers(const PacketData &pkt, const std::vector<LocalEndpoint> &locals) {
  ...
  if (had_failure && pkt.ty != SEDS_DT_TELEMETRY_ERROR && !error_endpoints.empty()) {
    const auto error_pkt =
        make_router_error_packet(std::move(error_endpoints), local_handler_error_message(failing_endpoint, failing_rc));
...
void router_receive_impl(SedsRouter &r, PacketData pkt, std::optional<int32_t> src_side) {
  ...
  if (pkt.ty == SEDS_DT_DISCOVERY_ANNOUNCE || pkt.ty == SEDS_DT_DISCOVERY_TIMESYNC_SOURCES) {
